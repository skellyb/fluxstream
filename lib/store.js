...
var FluxStore = function(def) {
     ...
     *                         payload or init for initial property value. If you
     *                         want to return a stream on each value use asyncMap.
    ...
    Store.prototype.configure = function(config) {
        ...
        Object.keys(config).forEach(function(prop) {
            ...
            var asyncMap = propConfig.asyncMap || function (val) { return val; };
            var map = propConfig.map || function (val) { return val; };

            var stream = propConfig.action.stream
                .flatMap(asyncMap)
                .map(map)
                .toProperty(map(config[prop].init));

            if (propConfig.inputAction) {

                initCache(propConfig, stream);

                // setup input action
                propConfig.inputAction.listen(function(payload) {
                    
                    var cacheVal = getCached.call(stream, propConfig.inputKey, payload);

                    // if cached data send it
                    // or send empty data to avoid displaying stale goods
                    if (cacheVal) {
                        propConfig.action(cacheVal);
                    } else if (propConfig.cacheKey) {
                        propConfig.action(propConfig.init);
                    }
...
function initCache(config, stream) {
    if (!config.cacheKey) return;

    stream.cache = {};
    config.action.listen( updateCache.bind(stream, config.cacheKey, config.cacheLimit) );
}
